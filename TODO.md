...
1. **継続タスクとSDK Continueの使い分け戦略** (最優先度 🔥)
2. **createApp関数の分割** (高優先度)
3. **エラーハンドリングの統一** (高優先度)
4. **フロントエンド状態管理の改善**
...
### バックエンドアーキテクチャの技術的負債
1. **createApp関数の肥大化** - 293行の巨大関数で責務が混在
2. **依存性の密結合** - DIコンテナ未実装でテスタビリティが低い
3. **エラーハンドリングの重複** - 各ルートで同様の処理が散在
...
1. **継続タスクとSDK Continueの使い分け戦略の実装**
   - **現状の機能**:
     - 継続タスク (`/api/tasks/{taskId}/continue`): 新規タスクとして前の結果をsystemPromptに含める
     - SDK Continue (`continueFromTaskId`): Claude Code SDKのセッション機能で同じセッションを継続
   - **実装事項**:
     - UI/UXでの明確な区別（「作業を続ける」vs「結果を参照して新規タスク」）
     - 自動判定ロジックの実装（時間経過、指示内容の関連性チェック）
     - コスト最適化の観点からのセッションサイズ管理
     - フロントエンドでの両オプション提示機能
   - **推奨使用パターン**:
     - 30分以内の追加作業 → SDK Continue
     - 時間経過後・異なる種類の作業 → 継続タスク
     - トークン使用量が多い場合の自動切り替え

2. **プロダクションビルドでlocalhost:5000でアクセスするとエラーになる。そもそも必要かどうかの検討**
   - 現状の問題点を調査
   - 本番環境でのlocalhost:5000へのアクセスの必要性を検討
   - 必要な場合は修正、不要な場合は適切なエラーハンドリングを実装

3. **Web UI アーキテクチャの再構築** ✅ (2025-01-23 完了)
...
5. **Claude Code SDK `continue`オプションの対応** ✅ (2025-01-25 session ID問題修正により一部解決)
...
7. **Claude Code SDK新機能の活用とセキュリティ強化**
   - **権限管理の強化**:
...
1. **createApp関数の分割** (Phase 2)
   - 現状: 293行に及ぶ巨大な関数で、複数の責務を持つ
   - 改善案: 以下の関数に分割
...
### Claude Code SDK session ID問題の修正 ✅ (2025-01-25)
...
### Web UI改善 ✅
- Svelte 5 + SvelteKitへの完全移行（2025-01-23）
