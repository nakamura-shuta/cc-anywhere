...
import { taskGroupStore } from "../../services/task-group-store.js";
...
const taskGroupsRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request) => {
      ...
      const allGroups = status
        ? taskGroupStore.getByStatus(status as any)
        : taskGroupStore.getAll();

      // Transform groups for response
      const groups = allGroups.map((record) => ({
        id: record.id,
        name: record.group.name,
        status: record.result.status,
        totalTasks: record.result.totalTasks,
        completedTasks: record.result.completedTasks,
        progress: record.progress.percentage,
        currentTask: record.currentTask,
        startedAt: record.startedAt.toISOString(),
        updatedAt: record.updatedAt.toISOString(),
        executionMode: record.group.execution.mode,
      }));
