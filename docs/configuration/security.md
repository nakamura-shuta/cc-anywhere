...
PathValidatorは以下の順序でパスを検証します（`backend/src/utils/path-validator.ts`参照）：

1. **絶対パス化**: 指定されたパスを`path.resolve()`で絶対パスに解決
2. **ホワイトリストチェック**: `REQUIRE_WHITELIST=true`の場合、許可リストに含まれるかチェック
3. **システムディレクトリチェック**: `STRICT_PATH_VALIDATION=true`の場合、システムディレクトリへのアクセスをブロック
4. **存在確認**: ディレクトリが実際に存在するか確認
5. **シンボリックリンク解決**: `fs.realpath()`でシンボリックリンクを解決し、解決後のパスに対して再帰的に検証を実行
