...
describe("Streaming WebSocket Integration", () => {
  ...
  it("should stream tool execution events", async () => {
    ...
    const taskId = taskQueue.add(taskRequest);
  ...
  it("should stream Claude responses", async () => {
    ...
    const taskRequest: TaskRequest = {
      instruction: "Explain what you're doing",
      options: {
        async: true,
  ...
  it("should calculate tool duration correctly", async () => {
    ...
    const receivedMessages: any[] = [];
    const toolEndEvents = receivedMessages.filter(m => 
      m.type === "task:tool:end" && m.payload.toolId === toolId
    );
